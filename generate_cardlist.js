...
<body>
  ...
  <main class="container">
    ...
    <p style="margin:-8px 0 16px;font-size:12px;color:var(--text-muted)">
      <a href="restrictions.html" style="color:var(--accent)">禁止・制限カード一覧はこちら</a>
    </p>
...
${SET_CHIP_GROUPS.map((g, gi) => {
  ...
  return `          <div class="filter-set-row"><div class="filter-chips">\n${allChip}${chips.join('\n')}\n          </div></div>`;
      ...
      <!-- バージョンフィルター（通常/パラレル、2026-07-25） -->
      <div class="filter-group">
        <span class="filter-group-label">バージョン / Version</span>
        <div class="filter-chips" id="filter-version">
          <button class="filter-chip active" data-value="all">全て</button>
          <button class="filter-chip" data-value="normal">通常のみ</button>
          <button class="filter-chip" data-value="parallel">パラレルのみ</button>
        </div>
      </div>
...
${generateNoscriptContent()}
  ...
  <script>
    ...
    var filterState = {
      ...
      version: 'all', // 'all', 'normal', 'parallel'（バージョン: 通常/パラレル、2026-07-25）
    ...
    function getFilteredCards() {
      var result = CARDS.filter(function(card) {
        ...
        // バージョンフィルタ（通常/パラレル、2026-07-25）: card.pl=1 がパラレル版
        if (filterState.version === 'normal' && card.pl) return false;
        if (filterState.version === 'parallel' && !card.pl) return false;
        // 規制状況フィルタ（指示書43）
    ...
    setupFilterGroup('filter-version', 'version', false);
