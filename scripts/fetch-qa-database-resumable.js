#!/usr/bin/env node
/**
 * scripts/fetch-qa-database-resumable.js（fetch-qa-database.js の再開型派生、2026-07-11）
 * 差分: ページキャッシュ(tmp/qa-cache-20260711) + --budget-ms による分割実行のみ。パーサ・マージは原本と同一。
...
const BACKUP_PATH = QA_PATH + '.bak-20260711-qa';
...
const CACHE_DIR = path.join(ROOT, 'tmp', 'qa-cache-20260711');
fs.mkdirSync(CACHE_DIR, { recursive: true });
function cacheKey(url) { return crypto.createHash('md5').update(url).digest('hex') + '.html'; }
...
async function crawl(startUrl, ctx, all) {
  ...
  while (url && guard++ < 50) {
    const cf = path.join(CACHE_DIR, cacheKey(url));
    let html, fromCache = false;
    if (fs.existsSync(cf)) { html = fs.readFileSync(cf, 'utf-8'); fromCache = true; }
    else {
      if (overBudget()) { const be = new Error('BUDGET'); be.budget = true; throw be; }
      html = await fetchHtml(url);
      fs.writeFileSync(cf, html, 'utf-8');
    }
...
async function main() {
  ...
  for (const url of subCategories) {
    ...
    if (be && be.budget) { console.log('[予算] 時間切れで中断。取得済みページはキャッシュ保存済み。再実行で続きから（書き込みなし）'); process.exit(3); }
    throw be;
  }
