...
`post-x-weekly-pack.js`(手動実行)が「今日のカード」の翌週7日分を**先計算**し、
X の予約投稿(Web版の手動機能)へ貼り付ける素材一式を出力する。
夜間バッチ・auto-news・ムーバーは一切変更していない。

- 出力先: `tmp/x-weekly-pack/<開始日>/`。日別 `NN-MMDD-<card_id>.txt`(投稿文・**UTF-8 BOMなし/CRLF/末尾改行なし**)、
  同名 `.jpg`(画像)、`予約チェックリスト.md`(**最後に書かれる完了マーカー**)
- **チェックリストが無いパックは生成途中。使わずに作り直す**(各日 `--unmark` → 出力先削除 → 再実行)
- 状態ファイル `.sched-run-tmp/x-daily-log.json` に `status:"manual_scheduled"` を記録する。
  post-x-daily は起動時の at-most-once ガードで**その日を無条件スキップ**するので二重投稿にならない
- パックを作らなかった週は従来どおり夜20:00の自動投稿が動く(自然なフォールバック)

### 選定は post-x-daily の `planDaily` を再利用する(再実装禁止)

- 指示書64 で `post-x-daily.js` の `module.exports` に **`loadInputs` を追加した**(実装は未変更)。
  変更前後で `--dry-run --date` 3日付の stdout がバイト一致することを機械証明済み
- パック側は日付順に `planDaily({...ctx, today: Di, state})` を呼び、
  結果をメモリ上の state へ `daily:<Di>` として反映してから次日へ進む**逐次シミュレーション**。
  これで 60 日ガード・週内重複回避が「毎晩実行した場合」と完全一致する
- 未来日キーは `diffDays` が負 → `<60` で 60 日ガードが効くため、週内の先日付どうしの衝突は数学的に起きない
- 選定は**生成時点のデータスナップショット**に基づく。週の途中で events.json が更新されても
  予約済みの文面は変わらない(運用として承認済み)

### 実行時間帯の鉄則

- **パック生成・`--unmark` とも 19:30〜21:30 JST は実行禁止**(スクリプト側でも自動で止まる)。
  夜間バッチが状態ファイルを書き終える前に読むと選定がずれるため
- 生成前に「実行日 〜 開始日前日」の `daily:` キーが**すべて存在すること**を検証し、
  欠けていればエラー中断する(ギャップ日検証)。この結果、**初回は 21:30 以降にしか実行できない**。
  2週目以降はパック週の日付が先に記録済みのため週末日中でも実行できる

### CLI(post-x-daily と同名のフラグに固定している)

```bash
node post-x-weekly-pack.js --dry-run                    # 選定・文面の表示のみ(出力・状態書込とも0件)
node post-x-weekly-pack.js                              # 翌日から7日分を生成
node post-x-weekly-pack.js --start 2026-08-10 --days 7 --out <dir>
node post-x-weekly-pack.js --unmark 2026-08-12          # 予約しなかった日を夜間自動へ戻す
```

- `--dry-run` / `--state-file` を post-x-daily と**同名**にしてあるのは、
  post-x-daily が require 時に自プロセスの argv を解釈するため。
  同名にすることで dry-run 中は post-x-daily 側の `writeStateAtomic` も throw する二重防御になる
- **`--date` / `--only` は意図的に定義していない**(post-x-daily 側が解釈してしまうため)。
  指定すると require より前にエラーで止める。未知のオプションもエラー
- `--unmark` は **`status:"manual_scheduled"` かつ翌日以降の日付**のみ許可。
  当日・過去日・posted/failed は拒否。**先に X 側の予約を取り消してから**実行すること
- `--out` は既存ならエラー中断(上書きしない)。`--days` は既定7・上限14
- 環境変数 `X_PACK_TEST_NOW` はテスト専用。**通常運用では設定しないこと**(時間帯ガードを無効化する)

### 書き込み順序(クラッシュ安全性のため厳守)

1. 日別 txt + jpg
2. 状態ファイルへ**最後に1回だけ**原子書き(tmp書き→rename)。`readState()` の返却オブジェクトを
   変異させて渡すので `version` 等の既存フィールドは保全される
3. `予約チェックリスト.md`(完了マーカー)

- 状態ファイルが破損していた場合は**中止のみ**。退避・再作成はしない(quarantine は夜間側の役割)
- 読み取り後に他プロセスが状態ファイルを書いていたら、書き込み前に検知して中断する

### 月曜の既知の挙動(許容)

パック週の月曜は、朝10:00の予約とは別に**夜20:00の週次ムーバーが従来どおり自動投稿**される。
daily がスキップされる関係で `sameNightCardIds` 除外が効かないため、
朝のカードが同日夜のムーバーにも載り得る。**仕様として許容**(チェックリストにもその旨を明記している)。

## NTC64大会の順位変換と集計上限(2026-08-12 追加・指示書70)

`series.json` で `type='ntc'` かつ **results が16件以上**のイベントは「64名定員NTC大会」と判定する
(`shared/ntc-rank-consolidator.js` の `isTargetEvent`)。

### 大会仕様(松岡さん説明 2026-08-12)

64名定員NTCは **32名定員イベントを2つ同時開催**する形式。5回戦を行い、**全勝した2名それぞれに優勝景品**を渡す
(32名定員なら全勝1名)。公式の結果表示は1〜16位だが、**実態は「1〜8位のセットが2つ」**。

### 集計の規則(全経路で統一)

- 順位は `Math.ceil(rank / 2)` で変換する(公式1〜16位 → 1〜8位・各順位2名)
- **集計上限は変換後 `rank <= 4` で統一**(＝公式1〜8位＝各セット4位まで＝1イベント8デッキ)
- **優勝は変換後 `rank === 1`。64名大会では1イベントあたり2件になるのが正しい**
- 32名定員大会(results 8件)は変換なし・`rank <= 4`(従来どおり)

`4` を `8` にしてはいけない。順位を半分に圧縮したうえで閾値も倍にすると打ち消し合い、
**「TOP4」が全件を通す**(TOP4 と 全て が同数になる)。2026-08-12 に外部ユーザーからこの不具合の
報告を受けて是正した。旧実装の「TOP4 → TOP8 拡張」(質問A / 追加確認1:B案 / 質問B:1)は**指示書70で撤回**。

### 同じ規則を持つ13か所(片方だけ直さないこと)

`meta.html` / `index.html` / `generate-events.js`(2) / `generate_cards.js`(2) / `generate-report.js`(3) /
`post-x-daily.js` / `scraper.js` / `x-weekly-banner.js` / `scripts/build-series-summary.js`

- `scripts/build-series-summary.js` だけは `? 8 : 4` の形ではなく **`top4_colors` を全件集計する別機構**。
  `t.rank > 4` で絞る(この `rank` は連結後の順位)。grep するときは `? 8 : 4` だけを見ないこと
- `index.html` の `consolidateNtcRank` には **必ず `isNtcType` を渡す**。渡さないとハードコードの
  `NTC_SERIES_IDS`(6226/6776/6791)だけで判定され、MISSION4 以降が変換されず top_stats 経路とずれる
- `scripts/build-ntc-june-interim.js` は公開済みの6月中間レポートを作った一過性スクリプトのため、
  意図的に旧規則(`rank<=8`)のまま据え置いている

### 8位タイの既知の制約

公式が「8位」を2名に付けたイベントでは `ceil(8/2)=4` で両者が変換後4位に入り、
そのセットの4位が3名・5位が1名になる。このため TOP4 は「優勝×4」より +1 になることがある
(2026年8月: 中国 41(=40+1) / 全地域 581(=580+1))。**発行元裁定2026-08-11によりバックログ低優先**。
